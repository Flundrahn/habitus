...
function Input(props: FieldHookConfig<IHabitFormValues>) {
  ...
  return (
    <div className="flex flex-col text-left relative">
      ...
      <input {...field} type={props.type} placeholder={props.placeholder} className="h-8 px-2 py-1 block bg-white text-sm placeholder-slate-400 outline-none focus:border-sky-500 border border-slate-300 disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 invalid:text-pink-600 invalid:placeholder-pink-600 focus:invalid:border-pink-500" />
      {meta.touched && meta.error ? (
        <div className="text-xs text-red-700 pl-2 absolute right-1">{meta.error}</div>
      ) : null}
    </div>
  );
}
...
export default function HabitForm({ habit, title, apiAction, button, setShowForm }: IHabitFormProps) {
